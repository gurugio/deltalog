    ...
    async fn do_get(
        ...
        match std::str::from_utf8(&ticket.ticket) {
            Ok(sql) => {
                ...
                let testdata = format!("file:///home/gurugio/study/deltalog/"); // So this could be a directory path including parquet files

                // register parquet file with the execution context
                ctx.register_parquet(
                    "logtable", //"alltypes_plain", What does this argument mean??? Table name?
                    &format!("{testdata}"),
